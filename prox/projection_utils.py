...
def compute_normals(depth_im, points, valid_idx, offset=3):
    h = depth_im.shape[0]
    w = depth_im.shape[1]
...
class Projection():
    ...
    def create_scan(self, mask, depth_im, color_im=None, mask_on_color=False, coord='color', TH=1e-2, default_color=[1.00, 0.75, 0.80], keypoints=None):
        ...
        normals = None
        if True:
            normals = compute_normals(depth_im, points, valid_idx, offset=3)
        ...
        return {'points':points, 'normals' : normals, 'colors':colors, 'keypoints3d': keypoints3d}
